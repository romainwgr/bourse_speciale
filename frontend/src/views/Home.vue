<template>
  <!-- Section du fond dynamique -->
  <div class="dynamic-background-wrapper">
    <!-- Image actuelle -->
    <div
      class="dynamic-background"
      :style="{ backgroundImage: `url(${currentBackground})`, transform: currentTransform }"
    ></div>

    <!-- Image suivante, pour l'animation de glissement -->
    <div
      class="dynamic-background next"
      :style="{ backgroundImage: `url(${nextBackground})`, transform: nextTransform }"
    ></div>
  </div>

  <!-- Section avec le fond noir et le contenu principal -->
...
export default {
  ...
  async created() {
    ...
    this.currentBackground = this.backgroundImages[this.backgroundIndex];
    this.nextBackground = this.backgroundImages[(this.backgroundIndex + 1) % this.backgroundImages.length];
    this.startBackgroundRotation();
  },
  methods: {
    startBackgroundRotation() {
      setInterval(() => {
        // Prépare l'animation : l'image actuelle glisse à gauche et la suivante arrive
        this.currentTransform = "translateX(-100%)";
        this.nextTransform = "translateX(0)";

        // Après l'animation, met à jour les images
        setTimeout(() => {
          this.backgroundIndex = (this.backgroundIndex + 1) % this.backgroundImages.length;
          this.currentBackground = this.nextBackground;
          this.nextBackground = this.backgroundImages[(this.backgroundIndex + 1) % this.backgroundImages.length];

          // Réinitialise les positions pour préparer la prochaine transition
          this.currentTransform = "translateX(0)";
          this.nextTransform = "translateX(100%)";
        }, 1000); // Correspond à la durée de la transition (1 seconde ici)
